...
Pricing is **family-based**, not per-ID. `priceForModel()` resolves an exact ID match first (so user overrides win), then falls back to the model's family (`familyForModel()` matches on `haiku`/`sonnet`/`opus`/`fable` substrings). This keeps `savedCents` correct for dated snapshots and Bedrock/Vertex `us.anthropic.*` IDs without a code change. Default tiers (`DEFAULT_MODELS`): haiku→`claude-haiku-4-5`, sonnet→`claude-sonnet-5`, opus→`claude-opus-5`, fable→`claude-fable-5`. Current generation (`FAMILY_PRICING`, $/1M in/out): Haiku 4.5 $1/$5, Sonnet 5 $3/$15 (standard; intro $2/$10 through 2026-08-31), Opus 4.6–4.8 and Opus 5 $5/$25, Fable 5 $10/$50. Opus 5 landed at 4.8's rate, so promoting the tier moved the model ID without rebasing a single historical savings figure — `models.test.ts` pins that. **Opus is NOT the old $15/$75 — drift here silently corrupts every savings figure.**
...
**Savings are counted, never subtracted** (`countedSavings()`). `baseline − actual` goes negative whenever routing lands *above* the baseline model — an opus route against the sonnet baseline — and reporting that signed delta made the ledger read as if the router had spent the user's money: a single opus call cancelled out dozens of haiku wins and the dashboard's "saved" card went red. The reported figure answers "how much did routing save", so a route at or above the baseline contributes **0**. The clamp is **per event, never on a total** (clamping the sum would still let one expensive route erase real savings before it got there) and it lives at two seams: `computeRouteCost` (so every header, log line, `RouteMeta` and `RouteEvent` carries a non-negative figure) and `foldOutcome` (so a `history.jsonl` written before the rule — negative deltas and all — folds to the same lifetime figure as a fresh one). The consequence to know: over-routing no longer shows up as negative savings, so read the tier distribution and the cost card for that, not the saved card.
...
src/proxy/
  ...
  format.ts     — `formatSavedCents()`: the one place a saved figure becomes text. An amount that rounds away renders as a neutral, unlabelled `$0.00` — never green `saved $0.00`, because a sub-cent figure is not a saving (same principle as unpriced models being loud rather than free). The negative/`extra` branch is kept as a formatter contract but is unreachable for live data now that `countedSavings` floors every figure at 0
