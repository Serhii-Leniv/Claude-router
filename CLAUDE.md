...
- Keeping the injected reminders let the *project's own docs* decide the tier (#34). Claude Code injects CLAUDE.md into the user turn as its own text block. Measured on `claude -p "yo"` inside this repo: 20,857 of the turn's 20,859 characters were injected context, and this CLAUDE.md contains "end-to-end" — a `DEPTH_MARKERS` hit — so a two-character greeting routed to **opus** via `agentic:depth-requested`. Any repo whose CLAUDE.md happens to contain a depth word paid opus rates on every turn.

Only the matched-pair reminder form is stripped; an unclosed tag is left alone rather than swallowing the turn. `buildAISnippet` head/tail-truncates the extractor's output, falling back to the joined text only when no user turn carries a request at all (`mode: 'ai'` bypasses the gates, so a pure tool_result history can reach it). Do not reintroduce a second extractor, and do not add a keyword gate that reads raw message content.
